...
function setSEpolicy()
{
	chconVersion_required=6
	chconVersion=$(chcon --version | grep coreutils | tr -d "[:alpha:][:space:]()")
	var1=$(awk 'BEGIN{ print "'${chconVersion_required}'"<"'${chconVersion}'" }')
